  ...
  bool keyed)
{
  ...
  if (occurrences != 2) {  // Name is namespaced: <package_name>::<msg/srv>::<type_name>
    RMW_SET_ERROR_MSG_WITH_FORMAT_STRING(
  ...
  type_name = _create_type_name(type_name.substr(0, last_pos - 2),
                                type_name.substr(last_pos, type_name.length() - last_pos));
  ...
  if (!fastdds_type) {
    ...
    tsupport->setName(type_name.c_str());  // Because we're using TypeSupport_cpp, we need to do this
